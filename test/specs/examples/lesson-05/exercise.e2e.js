...
describe('Login Page', async () => {
  ...
  // it.only('should registrace', async () => {
  //   const text = $('h1');
  //   await browser.reloadSession();
  //   await browser.url('/registrace');

  //   await expect(text).toHaveText('Registrace');
  // });
});
...
describe('Applications Page', async () => {
  ...
  it('should list all applications', async () => {
    ...
    for (const row of rows) {
      ...
      await expect(cols[1]).toHaveText(
        /(\d{2}.\d{2}.\d{4}|\d{2}.\d{2}. - \d{2}.\d{2}.\d{4})/
      );
      await expect(cols[2]).toHaveText(
        /(Bankovní převod|FKSP|Hotově|Složenka)/
      );
      // nebo
      await expect(cols[2]).toHaveText([
        'Bankovní převod',
